...
`
const addToCartToolServer = addToCartToolDef.server(((args: { guitarId: string; quantity: number }) => ({
  success: true,
    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          ...
          const err = error as Error & { status?: number; statusText?: string; code?: string; type?: string }
